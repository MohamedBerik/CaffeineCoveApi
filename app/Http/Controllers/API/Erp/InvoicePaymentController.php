...
class InvoicePaymentController extends Controller
{
    ...
    public function applyCustomerCredit(Request $request, $invoiceId)
    {
        $companyId = $request->user()->company_id;

        return DB::transaction(function () use ($request, $invoiceId, $companyId) {
            $invoice = Invoice::where('company_id', $companyId)
                ->lockForUpdate()
                ->findOrFail($invoiceId);
            ...
            if ($invoice->status === 'cancelled') {
                return response()->json([
                    'msg' => 'Cannot apply credit to cancelled invoice',
                ], 422);
            }

            // 1) invoice paid from direct payments
            $totalApplied = Payment::where('company_id', $companyId)
                ->where('invoice_id', $invoice->id)
                ->sum('applied_amount');

            // 2) refunded from invoice-applied payments
            $totalRefunded = DB::table('payment_refunds')
                ->join('payments', 'payments.id', '=', 'payment_refunds.payment_id')
                ->where('payments.company_id', $companyId)
                ->where('payments.invoice_id', $invoice->id)
            ...
            // 3) credit already applied to this invoice
            $totalCreditApplied = DB::table('customer_credits')
                ->where('company_id', $companyId)
                ->where('invoice_id', $invoice->id)
                ->where('type', 'debit')
            ...
            // available customer credit = credits - debits
            ...
            if ($availableCredit <= 0) {
                return response()->json([
                    'msg' => 'No available customer credit',
                    'available_credit' => 0,
                ], 422);
            }
            ...
            DB::table('customer_credits')->insert([
                ...
                'entry_date'  => now(),
                'description' => 'Customer credit applied to invoice #' . $invoice->number,
                'created_by'  => $request->user()->id ?? null,
                'created_at'  => now(),
                'updated_at'  => now(),
            ]);

            // AR ledger: credit receivable because invoice got settled by customer credit
            CustomerLedgerEntry::create([
                'company_id'  => $companyId,
                'customer_id' => $invoice->customer_id,
                'invoice_id'  => $invoice->id,
            ...
            AccountingService::createEntry(
                $invoice,
                'Customer credit applied to invoice #' . $invoice->number,
                [
                    [
            ...
            $customerCreditBalance = max(0, $availableCredit - $creditToApply);

            return response()->json([
                'msg' => 'Customer credit applied successfully',
                'invoice_status' => $status,
                'credit_applied' => (float) $creditToApply,
                'net_paid' => (float) $netAfter,
                'remaining' => (float) $remainingAfter,
