...
class InvoiceController extends Controller
{
    ...
    private function buildInvoiceResponse(Invoice $invoice, int $companyId): array
    {
        ...
        /*
    |--------------------------------------------------------------------------
    | 1) Invoice Payment Totals
    |--------------------------------------------------------------------------
    | total_paid        = direct applied cash/card/bank payments only
    | total_refunded    = refunds against invoice-applied amounts only
    | total_credit_applied = customer credit consumed on this invoice
    | net_paid          = applied - refunded + credit applied
    | remaining         = total - net_paid
    */
        ...
        /*
    |--------------------------------------------------------------------------
    | 2) Customer Total Credit Balance
    |--------------------------------------------------------------------------
    | customer_credit_balance = all issued customer credits - all used credits
    |--------------------------------------------------------------------------
    */
        ...
        $customerCreditBalance = max(
            0,
            $totalCustomerCreditIssued - $totalCustomerCreditUsed
        );

        /*
    |--------------------------------------------------------------------------
    | 3) Credit Issued From This Invoice Payments Only
    |--------------------------------------------------------------------------
    | This is not the customer's total balance.
    | This is only credit generated by overpayments made on THIS invoice.
    |--------------------------------------------------------------------------
    */

        $totalCreditIssuedFromThisInvoice = (float) $payments->sum(
            fn($p) => (float) $p->credit_amount
        );
        ...
        /*
    |--------------------------------------------------------------------------
    | 4) Actual Cash Received On This Invoice
    |--------------------------------------------------------------------------
    */
        ...
        return [
            ...
            'payments' => $payments->map(function ($p) {
                $refInv = (float) $p->refunds
                    ->where('applies_to', 'invoice')
                    ->sum('amount');

                $refCr = (float) $p->refunds
                    ->where('applies_to', 'credit')
                    ->sum('amount');
